...
function Invoke-AwsJson {
    ...
    $Output = & aws @Arguments --profile $ProfileName --region $Region --output json 2>&1
...
function Test-ExpectedTags {
    ...
    foreach ($Key in $RequiredTags.Keys) {
        if (-not $TagMap.ContainsKey($Key) -or $TagMap[$Key] -ne $RequiredTags[$Key]) {
            $TagsValid = $false
        }
    }
...
try {
    ...
    $null = Invoke-AwsJson -Arguments @("sts", "get-caller-identity")
    ...
    if ($Vpc.State -eq "available" -and $Vpc.CidrBlock -eq $VpcCidr) {
    ...
    $DnsSupport = Invoke-AwsJson -Arguments @(
        "ec2", "describe-vpc-attribute", "--vpc-id", $VpcId,
    ...
    if ($DnsSupport.EnableDnsSupport.Value -and $DnsHostnames.EnableDnsHostnames.Value) {
        Write-Pass "DNS support and DNS hostnames are enabled."
    }
    else {
        Write-Fail "DNS support or DNS hostnames are not enabled."
    }
    Test-ExpectedTags -Tags $Vpc.Tags -ResourceLabel "VPC"
    ...
    foreach ($ExpectedSubnet in @(
        @{ Name = $SubnetAName; Cidr = $SubnetACidr; Zone = $AvailabilityZoneA },
        @{ Name = $SubnetBName; Cidr = $SubnetBCidr; Zone = $AvailabilityZoneB }
    )) {
        $Matches = @($Subnets | Where-Object {
            ($_.Tags | Where-Object { $_.Key -eq "Name" -and $_.Value -eq $ExpectedSubnet.Name }).Count -eq 1
        })
        ...
        Test-ExpectedTags -Tags $Subnet.Tags -ResourceLabel $ExpectedSubnet.Name
    }
    ...
    $NamedGateways = @($Gateways | Where-Object {
        ($_.Tags | Where-Object { $_.Key -eq "Name" -and $_.Value -eq $InternetGatewayName }).Count -eq 1
    })

    if ($NamedGateways.Count -eq 1) {
        $InternetGateway = $NamedGateways[0]
        $InternetGatewayId = $InternetGateway.InternetGatewayId
        Write-Pass "Internet Gateway is attached to the VPC."
        Test-ExpectedTags -Tags $InternetGateway.Tags -ResourceLabel "Internet Gateway"
    }
    else {
    ...
    $ExpectedSubnetIds = @(
        ($Subnets | Where-Object { $_.CidrBlock -eq $SubnetACidr }).SubnetId,
        ($Subnets | Where-Object { $_.CidrBlock -eq $SubnetBCidr }).SubnetId
    )
    $AssociatedSubnetIds = @($RouteTable.Associations | Where-Object {
        -not [string]::IsNullOrWhiteSpace($_.SubnetId)
    } | ForEach-Object { $_.SubnetId })

    $AssociationsValid = $true
    foreach ($ExpectedSubnetId in $ExpectedSubnetIds) {
        if ([string]::IsNullOrWhiteSpace($ExpectedSubnetId) -or $ExpectedSubnetId -notin $AssociatedSubnetIds) {
            $AssociationsValid = $false
        }
    }

    if ($AssociationsValid -and $AssociatedSubnetIds.Count -eq 2) {
    ...
    Test-ExpectedTags -Tags $RouteTable.Tags -ResourceLabel "Public route table"
    ...
    if (@($SecurityGroup.IpPermissions).Count -eq 0) {
        Write-Pass "Security Group contains no ingress rules."
    }
    else {
        Write-Fail "Security Group contains unexpected ingress rules."
    }
    Test-ExpectedTags -Tags $SecurityGroup.Tags -ResourceLabel "Security Group"
