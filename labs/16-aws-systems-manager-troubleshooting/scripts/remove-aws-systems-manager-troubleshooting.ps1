...
try {
    Write-Host ""
    Write-Host "=== Cleanup do Lab 16 ===" -ForegroundColor Cyan
    ...
    if ($instances.Count -eq 1) {
        ...
        Write-Host "[INFO] Nenhuma instância ativa do Lab 16."
    }
    ...
    if ($vpcs.Count -ne 1) {
        throw "A VPC compartilhada do Lab 08 não foi encontrada de forma única."
    }
    $vpcId = $vpcs[0].VpcId
    ...
    if ($subnets.Count -ne 1) {
        throw "A sub-rede compartilhada do Lab 08 não foi encontrada de forma única."
    }
    $subnetId = $subnets[0].SubnetId

    if ($null -ne $instance -and $instance.SubnetId -ne $subnetId) {
        throw "A instância está em uma sub-rede inesperada. Remoção recusada."
    }
    ...
    if ($groups.Count -eq 1) {
        ...
        Write-Host "[INFO] Security Group do Lab 16 ausente."
    }
    ...
    if ($null -ne $roleResponse) {
        ...
        Write-Host "[INFO] IAM Role do Lab 16 ausente."
    }
    ...
    if ($null -ne $profileResponse) {
        ...
        Write-Host "[INFO] Instance Profile do Lab 16 ausente."
    }
    ...
    if ($null -ne $group) {
        ...
        foreach ($networkInterface in $interfaces) {
            if (
                ...
                throw (
                    ...
                    "que não pertence à instância do Lab 16. " +
                    "Remoção recusada."
                )
            }
        }
        ...
        if ($externalReferences.Count -gt 0) {
            throw (
                "Outro Security Group referencia o grupo do Lab 16. " +
                "Remoção recusada."
            )
        }
    }
    ...
    if (
        ...
        Write-Host ""
        Write-Host "[OK] Nenhum recurso exclusivo do Lab 16 para remover." `
            -ForegroundColor Green
        Write-Host "[OK] VPC $vpcId e sub-rede $subnetId preservadas."
        return
    }
    ...
    if (@($remainingGroups.SecurityGroups).Count -ne 0) {
        throw "O Security Group do Lab 16 ainda está presente."
    }
    ...
    if ($null -ne $remainingRole) {
        throw "A IAM Role do Lab 16 ainda está presente."
    }
    ...
    if ($null -ne $remainingProfile) {
        throw "O Instance Profile do Lab 16 ainda está presente."
    }

    $remainingInstances = Get-AwsJson -Arguments @(
        "ec2", "describe-instances", "--filters",
        "Name=tag:Name,Values=$InstanceName",
        "Name=instance-state-name,Values=pending,running,stopping,stopped,shutting-down"
    )
    $activeInstances = @(
        $remainingInstances.Reservations | ForEach-Object { $_.Instances }
    )
    if ($activeInstances.Count -ne 0) {
        throw "Ainda existe uma instância ativa com o nome do Lab 16."
    }

    $remainingVpc = Get-AwsJson -Arguments @(
        "ec2", "describe-vpcs", "--vpc-ids", $vpcId
    )
    $remainingSubnet = Get-AwsJson -Arguments @(
        "ec2", "describe-subnets", "--subnet-ids", $subnetId
    )
    if (
        @($remainingVpc.Vpcs).Count -ne 1 -or
        @($remainingSubnet.Subnets).Count -ne 1 -or
        $remainingSubnet.Subnets[0].VpcId -ne $vpcId
    ) {
        throw "A VPC ou a sub-rede compartilhada não foi confirmada após o cleanup."
    }
